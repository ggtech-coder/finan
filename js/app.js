...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Sidebar collapse (desktop apenas — mobile usa hamburger + transform)
  ...
  function setSidebarCollapsed(collapsed) {
    if (!isDesktop()) return; // nunca aplica no mobile
  ...
  // Restaura estado salvo (só em desktop)
  if (isDesktop() && localStorage.getItem('sidebarCollapsed') === '1') {
    setSidebarCollapsed(true);
  }

  // Clique no botão de collapse
  collapseBtn?.addEventListener('click', () => {
    setSidebarCollapsed(!sidebar?.classList.contains('collapsed'));
  });

  // Ao redimensionar: remove collapsed se virar mobile
