App.Dashboard = (() => {
  ...
  function refresh() {
    ...
    const totalExpense = expense.reduce((a,b)=>a+Number(b.value||0), 0);
    ...
    const forecast = totalIncome - totalExpense;

    const set = (id, val) => { const e = document.getElementById(id); if (e) e.textContent = val; };
    set('kpiIncome', Utils.money(totalIncome));
    set('kpiExpense', Utils.money(totalExpense));
    ...
    // Emergency reserve from patrimony
    DB.getAll('patrimony').then(p => {
      const emergency = p.filter(x=>x.type==='poupanca'||x.type==='conta_corrente')
    ...
    renderUpcoming(all);
    renderDashboardChart(totalIncome, totalPaid, totalPending, overdue.reduce((a,b)=>a+Number(b.value||0),0));

    // KPI card color for overdue
    const kpiOverdueCard = document.querySelector('.kpi-overdue');
    if (kpiOverdueCard) {
      kpiOverdueCard.style.borderLeftColor = overdue.length > 0 ? '#ef4444' : '';
    }
  }
  ...
  function renderUpcoming(all) {
    ...
    const upcoming = all
      ...
      .filter(t => {
        const d = Utils.parseDate(t.dueDate);
        return d >= today || t.status === 'overdue';
      })
  ...
  function renderDashboardChart(income, paid, pending, overdue) {
    ...
    _chart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          data: [income, paid, pending, overdue],
